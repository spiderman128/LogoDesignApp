...
var Path = PathItem.extend(
  ...
  },
  {
    ...
    isFullySelected: function () {
      var length = this._segments.length;
      return (
        this.isSelected() && length > 0 && this._segmentSelection === length * 7
      );
    },
    ...
    function divideLocations(locations, include, clearLater) {
      ...
      for (var i = locations.length - 1; i >= 0; i--) {
        ...
        if (curve) {
          if (curve !== prevCurve) {
            clearHandles =
              !curve.hasHandles() || (clearLookup && clearLookup[getId(curve)]);
            renormalizeLocs = [];
            prevTime = null;
            prevCurve = curve;
      ...
      function addWinding(v) {
        ...
        var t =
            po === o0
              ...
              : 1,
          a =
            t === 0 ? a0 : t === 1 ? a3 : Curve.getPoint(v, t)[dir ? "y" : "x"],
          winding = o0 > o3 ? 1 : -1,
...
var PathText = TextItem.extend({
  ...
  setCursorPosition: function (position) {
    ...
    if (lineMatch) {
      ...
      if (lineGlyphs) {
        let offset =
          lineMatch.line && lineMatch.line.alignOffset
            ? lineMatch.line.alignOffset
            : 0;
        let closestPosition = this.closest(position.x + offset, lineGlyphs);
  ...
  drawCursor: function (ctx) {
    ...
    let offset =
      this.layout.lines[position.index] &&
      this.layout.lines[position.index].alignOffset
        ? this.layout.lines[position.index].alignOffset
        : 0;
    ctx.save();
  ...
  _draw: function (ctx, param, viewMatrix) {
    ...
    for (let i = 0; i < lines.length; i++) {
      let line = lines[i];
      ctx.save();
      ctx.translate(line.alignOffset, line.height * i);
...
var Style = Base.extend(
  ...
  })(),
  {
    ...
    getSpacing: function getSpacing() {
      var spacing = getSpacing.base.call(this);
      return spacing;
    }, // custom
  }
);
...
new (function () {
  ...
  function exportRaster(item, options) {
    ...
    attrs.href =
      (options.embedImages == false && image && image.src) || item.toDataURL();
  ...
  // custom
  function exportPathText(item) {
    let attrs = getTransform(item._matrix);
    let g = SvgElement.create("g", attrs, formatter);
    let font = item.getFont();
    if (font) {
      let lines = item.layout.lines;
      for (let i = 0; i < lines.length; i++) {
        let line = lines[i];
        let pathAttrs = new Base({
          transform: `translate(${line.alignOffset}, ${
            line.height * i
          }) scale(${item.layout.scale}, ${-item.layout.scale})`,
          d: font.typr.U.pathToSVG(line.draw),
        });
        // set stroke-width with scale
        if (!isNaN(item._style.getStrokeWidth())) {
          pathAttrs["stroke-width"] =
            item._style.getStrokeWidth() / item.layout.scale;
        }
        g.appendChild(SvgElement.create("path", pathAttrs, formatter));
      }
    }
    return g;
  }
  ...
  var exporters = {
    ...
    PathText: exportPathText, // custom
  };
...
new (function () {
  ...
  function applyAttributes(item, node, isRoot) {
    ...
    Base.each(attributes, function (apply, name) {
      ...
      item =
        (value !== undefined && apply(item, value, name, node, styles)) || item;
    });
    return item;
  }
    ...
    function onLoad(svg) {
      try {
        var node =
          typeof svg === "object"
            ? svg
            : new DOMParser().parseFromString(svg.trim(), "image/svg+xml");
        if (!node.nodeName) {
          node = null;
