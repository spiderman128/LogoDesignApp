...
Typr["T"].CFF = {
  ...
  glyphBySE: function (
    cff,
    charcode // glyph by standard encoding
  ) {
...
Typr["U"] = {
  ...
  shapeToPath: function (font, shape, spacing, scale = 1) {
    var tpath = { cmds: [], crds: [] };
    var positionMap = [];
    var x = 0,
      y = 0;
    ...
    for (var i = 0; i < shape.length; i++) {
      var it = shape[i];
      let loopSpacing = i === shape.length - 1 ? 0 : spacing;
      var path = Typr["U"]["glyphToPath"](font, it["g"]),
        crds = path["crds"];
      ...
      x += it["ax"] + loopSpacing;
      y += it["ay"];
      positionMap.push({ x: x * scale, width: it["ax"] * scale });
    }
    return {
      draw: { cmds: tpath.cmds, crds: tpath.crds },
  ...
  codeToGlyph: function (font, code) {
    ...
    var tind = -1,
      pps = ["p3e10", "p0e4", "p3e1", "p1e0", "p0e3", "p0e1" /*,"p3e3"*/];
    ...
    if (
      ...
      [
        0x9, 0xa, 0xb, 0xc, 0xd, 0x20, 0x85, 0xa0, 0x1680, 0x2028, 0x2029,
        0x202f, 0x3000, 0x180e, 0x200b, 0x200c, 0x200d, 0x2060, 0xfeff,
      ].indexOf(code) == -1 &&
        ...
        if (i == i0) {
          if (onCurve) {
            ...
            if (prOnCurve) P.MoveTo(p, gl.xs[pr], gl.ys[pr]);
              ...
              P.MoveTo(
                p,
                Math.floor((gl.xs[pr] + x) * 0.5),
                Math.floor((gl.ys[pr] + y) * 0.5)
              );
          }
        }
        if (onCurve) {
          ...
          if (nxOnCurve) P.qCurveTo(p, x, y, gl.xs[nx], gl.ys[nx]);
            ...
            P.qCurveTo(
              ...
              x,
              y,
              Math.floor((x + gl.xs[nx]) * 0.5),
              Math.floor((y + gl.ys[nx]) * 0.5)
            );
        }
      }
      P.ClosePath(p);
  ...
  SVG: (function () {
    ...
    function _readTrnsAttr(fnc, vls) {
      ...
      if (false) {
        ...
        if (vls.length == 1) M.translate(m, vls[0], 0);
        else M.translate(m, vls[0], vls[1]);
      } else if (fnc == "scale") {
        if (vls.length == 1) M.scale(m, vls[0], vls[0]);
        else M.scale(m, vls[0], vls[1]);
      } else if (fnc == "rotate") {
        var tx = 0,
          ty = 0;
        if (vls.length != 1) {
    ...
    function _tokens(d) {
      var ts = [],
        ...
        rn = false,
        cn = "",
        pc = ""; // reading number, current number, prev char
      while (off < d.length) {
        ...
        if (cmu == "Z") {
          ...
          for (var j = 0; j < reps; j++) {
            ...
            if (false) {
              ...
              var co = Math.max(
                crds.length - (cmds[cmds.length - 1] == "C" ? 4 : 2),
                oldo
              );
              var x1 = x + x - crds[co],
                y1 = y + y - crds[co + 1];
              ...
              var angl = function (ux, uy, vx, vy) {
                ...
                return (
                  (ux * vy - uy * vx >= 0 ? 1 : -1) *
                  Math.acos(Math.max(-1, Math.min(1, num)))
                );
              };
